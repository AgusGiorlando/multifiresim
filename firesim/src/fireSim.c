...
int main(int argc, char **argv)
{
    ...
    for (cell = 0; cell < cells; cell++)
    {
        ...
        {
            initial_ignitions++;
            printf("Ignition started in cell %d with time %f\n", cell, ignMap[cell]);
        }
    }

    if (initial_ignitions == 0)
    {
        fprintf(stderr, "Error: No cells ignited initially.\n");
        return 1; 
    }
    ...
    /* NOTE 5: create a standard fuel model catalog and a flame length table. */
    ...
    for (n = 0; n < 8; n++)
    {
        nDist[n] = sqrt(nCol[n] * CellWd * nCol[n] * CellWd + nRow[n] * CellHt * nRow[n] * CellHt);
        nAzm[n] = n * 45.;
    }

    fprintf(stderr, "Distancia a celdas vecinas calculada\n");

    /* NOTE 6: find the earliest (starting) ignition time. */
    ...
    /* NOTE 7: loop until no more cells can ignite or fire reaches an edge. */
    // atEdge = 0;
    while (timeNext <= endtime /*&& ! atEdge*/)
    {
        ...
        /* NOTE 8: examine each ignited cell in the fuel array. */
        for (cell = 0, row = 0; row < Rows; row++)
        {
            for (col = 0; col < Cols; col++, cell++)
            {
                ...
                /* NOTE 9: determine basic fire behavior within this cell. */
                ...
                /* NOTE 10: examine each unignited neighbor. */
                //? Propagacion del fuego a las celdas vecinas
                for (n = 0; n < 8; n++)
                {
                    // Find the neighboring cell's position
                    nrow = row + nRow[n];
                    ncol = col + nCol[n];
                    ...
                    // Calculate spread at the azimuth and check if it spreads to this neighbor
                    Fire_SpreadAtAzimuth(catalog, modelNumber, nAzm[n], FIRE_NONE);
                    fpm = Fuel_SpreadAny(catalog, modelNumber);

                    if (fpm > Smidgen)
                    {
                        // Calculate the time it takes for fire to reach the neighbor
                        minutes = nDist[n] / fpm;
                        ignTime = timeNow + minutes;
                        

                        printf("Ignition time calculated: %f = %f + %f for cell [%d, %d]\n", ignTime, timeNow ,minutes, nrow, ncol);

                        /* Check if the ignition time is less than the current cell's ignition time */
                        if (ignTime < ignMap[ncell])
                        {
                            printf("Updating ignition map for cell [%d, %d]: old time: %f, new time: %f\n",
                                   nrow, ncol, ignMap[ncell], ignTime);
                            ignMap[ncell] = ignTime;
                        }

                        /* Track the next cell ignition time */
                        if (ignTime < timeNext)
                        {
                            timeNext = ignTime;
                        }
                    ...
                    {
                        printf("No propagation to cell [%d, %d], fpm = %f\n", nrow, ncol, fpm);
                    }
    ...
    /* NOTE 11: if requested, save the ignition & flame length maps. */
    if (argc > 1)
    {
